import sqlite3
from database import SECRET_KEYS
...
def build_jira_config(db: sqlite3.Connection, *, cache: bool | None = None) -> dict:
    """Build the jira config dict from the database.

    Args:
        cache: Override the stored cache setting. Pass False to disable caching
               (e.g. for test-connection / status checks). Defaults to the stored value.
    """
    raw = get_raw_config(db)
    stored_cache = raw.get("jira_cache_enabled", "true").lower() == "true"
    return {
        ...
        "cache": stored_cache if cache is None else cache,
        "oauth": {
