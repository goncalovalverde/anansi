...
def get_charts(dataset_id: str, db: sqlite3.Connection = Depends(get_db)):
    ...
    if status in ("pending", "loading"):
        raise HTTPException(status_code=409, detail=f"Dataset is not ready yet (status: {status})")
    if status == "failed":
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="Chart rendering failed - check server logs for details")
